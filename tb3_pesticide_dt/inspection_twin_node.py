#!/usr/bin/env python3
"""Digital entity for the pesticide inspection proof of concept."""
...
class InspectionTwinNode(Node):
    ...
    def _declare_parameters(self):
        ...
        self.declare_parameter("residue_overuse_threshold", 0.50)
    ...
    def on_inspection_request(self, msg: String):
        ...
        self.get_logger().info(f"Queued hyperspectral inspection for {zone_id}")
    ...
    def publish_inspection_result(self, request: Dict):
        ...
        if camera_health == "failed":
            ...
            residue_index = zone.residue_index
            threshold = float(self.get_parameter("residue_overuse_threshold").value)
            status = classify_residue(residue_index, threshold)
